        ...
        parameter(im=254,jm=225,nhrd=14)
        real uu(im,jm),wusurf(im,jm),wvsurf(im,jm)
        real vv(im,jm),taux(im,jm),tauy(im,jm),wind(im,jm)
        
         time=0.1041666698E-01
c        time=0.2083333395E-01
c        time=0.3125000000E-01
        time=0.4166666791E-01
        time=0.5208333582E-01+0.1041666698E-01
        time=3.0

        call newwind(time,uu,vv,wusurf,wvsurf,taux,tauy)

        do 68 i=1,im
        do 68 j=1,jm
          wind(i,j)=sqrt(uu(i,j)**2+vv(i,j)**2)
c          if(wind(i,j).gt.20)print*,wind(i,j),taux(i,j),tauy(i,j)
68      continue


       open(45,file='speed.dat',form='unformatted')
       write(45)speed
       open(46,file='taux.dat',form='unformatted')
       write(46)taux
       open(47,file='tauy.dat',form='unformatted')
       write(47)tauy
...
c
        ...
        integer month(nhrd),day(nhrd),year(nhrd),hour(nhrd),
     &   min(nhrd),sec(nhrd)
        double precision sjul,jul(nhrd),jul1,jul2,juli
        character*5 hur
        ...
        do k=1,nhrd
          jul(k)=sjul-4.50+(k-1)*6.
c may be this should change later ....
          if(jul(k).eq.juli)juli=juli+0.02   
          call greg(jul(k),1,2,month(k),day(k),year(k),
     &              hour(k),min(k),sec(k),ier)
        enddo
