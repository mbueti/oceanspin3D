...
c
        ...
        enddo
c
cRMY 12/13/06: Set Katrina-specific values for calling frlvstress ------
        frlvhstart=69.
        frlvhend=168.
cRMY        if((time*24.0).lt.float(inowindh)) then
        if((time*24.0).ge.frlvhstart.and.(time*24.0).le.frlvhend) then
cRMY          call wind(pathname,startdate)
          call frlvstress(time,frlvhstart,wusurf,wvsurf,taux,tauy)
          if(pathname1.ne.pathname) then
cRMY            call wind(pathname1,startdate)
            call frlvstress(time,frlvhstart,wusurf,wvsurf,taux,tauy)
          end if
cRMY 12/13/06: End of changes for calling frlvstress -------------------
